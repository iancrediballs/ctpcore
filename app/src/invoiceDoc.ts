...
export type DocCompany = {
  ...
  // Optional because the desktop's company row does not carry them. Both are
  // owner-edited settings on the web (Settings → Company); a blank one prints
  // nothing, never a placeholder.
  reg_no?: string | null; bank_details?: string | null;
};
export type DocLine = {
...
export function buildDocHTML(order: DocOrder, company: DocCompany): string {
  ...
  const compLines = [
    ...
    company.reg_no ? "Reg No: " + company.reg_no : null,
    company.tax_id ? "VAT No: " + company.tax_id : null,
  ].filter(Boolean).map((x) => `<div>${esc(x as string)}</div>`).join("");
  const fulfilledFrom = order.location_code ? `<br>Fulfilled from: ${esc(order.location_code)}` : "";
  // Bank details are typed as lines in Settings; keep the line breaks.
  const bank = company.bank_details?.trim()
...
<body>
  ...
  ${bank}
  ${company.terms ? `<div class="terms"><div class="lbl">Terms</div>${esc(company.terms).replace(/\r?\n/g, "<br>")}</div>` : ""}
