...
export type Part = {
  ...
  sku: string;
  name: string;
  side: string;                 // "L/H" | "R/H" | "CENTRE" | "BOTH" | "-"
  inv_pn: string;
  cat_pn: string;
  locator: string;
  section: string;
  loc: string;                  // warehouse bin, e.g. "A1-a"
  qty: number;                  // on hand (derived from the ledger)
  cost: number;                 // landed, ZAR major units
  list: number;                 // list, ZAR major units
  dwg: string;
  photo: boolean;
  notes: string;
  refs: string[];               // cross-references parsed out of notes
  flag: "no_name" | null;
};

/** Raw shape returned by the `jefrey_catalogue` Tauri command. */
export type RustPart = {
  id: number; sku: string; name: string;
  side: string | null; inv_pn: string | null; cat_pn: string | null;
  locator: string | null; section: string | null; loc: string | null;
  qty: number; cost_minor: number | null; list_minor: number | null;
  dwg: string | null; has_photo: boolean; notes: string | null;
  match_status: string | null;
};

export type Candidate = { part: Part; score: number; why: string; contested?: boolean };
export type TraceStep = { step: string; detail: string };
export type Band = "confirmed" | "probable" | "contested" | "uncertain" | "unmatched";
export type Row = {
  key: string; raw: string; qty: number; phrase: string; side: string | null;
  trace: TraceStep[]; candidates: Candidate[]; chosen: number; status: Band;
};
...
export function fromRust(rows: RustPart[]): Part[] {
  const SIDE_MAP: Record<string, string> = { L: "L/H", R: "R/H", C: "CENTRE", B: "BOTH" };
  return rows.map((r) => {
    ...
    return {
      ...
      photo: !!r.has_photo,
      notes,
      refs: (notes.match(/Ref\.?\s*([0-9A-Za-z-]{6,})/g) || []).map((m) =>
        m.replace(/Ref\.?\s*/i, "")
      ),
      flag: name ? null : ("no_name" as const),
    };
  });
}
...
export const canonPN = (s: string): string => (s || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
...
export const basePN = (s: string): string => {
  const c = canonPN(s);
...
export const samePart = (a: string, b: string): boolean => {
  const x = canonPN(a), y = canonPN(b);
  if (!x || !y) return false;
  if (x === y) return true;
  if (basePN(x) === basePN(y) && basePN(x).length >= 7) return true;
...
const ALIAS_INDEX = ALIASES.flatMap(([phrases, canon, kind]) =>
  phrases.map((p) => ({ p, canon, kind, len: p.length }))
).sort((a, b) => b.len - a.len);

/** Expand shop vernacular into catalogue language and report what fired. */
export function expand(text: string): { text: string; fired: string[] } {
...
export type AliasRow = { phrase: string; part_id: number; polarity: number; hits: number };

export class Memory {
  positive = new Map<string, { partId: number; hits: number }>();
  ...
  load(rows: AliasRow[]) {
    ...
    this.negative.clear();
    rows.forEach((r) => {
      if (r.polarity < 0) this.negative.add(`${r.phrase}::${r.part_id}`);
      else this.positive.set(r.phrase, { partId: r.part_id, hits: r.hits });
    });
  }
  learn(phrase: string, partId: number) {
  ...
  reject(phrase: string, partId: number) {
    this.negative.add(`${normalise(phrase)}::${partId}`);
  }
  recall(phrase: string) {
    return this.positive.get(normalise(phrase)) || null;
  }
  isRejected(phrase: string, partId: number) {
...
export const trigrams = (s: string): Set<string> => {
  const t = `  ${normalise(s)} `;
...
export const dice = (a: Set<string>, b: Set<string>): number => {
...
export const coverage = (req: Set<string>, cand: Set<string>): number => {
  if (!req.size) return 0;
  let hit = 0;
  req.forEach((g) => { if (cand.has(g)) hit++; });
  return hit / req.size;
};

export const relevance = (req: Set<string>, cand: Set<string>): number =>
  Math.max(dice(req, cand), coverage(req, cand) * 0.92);
...
const CHATTER =
  /^(hi|hii|hello|hey|dear|thanks|thank you|regards|kind regards|cheers|morning|good (morning|day|afternoon|evening)|please|pls|urgent|asap|see below|as discussed|quote|quotation)\b/i;

export function isChatter(line: string): boolean {
  const t = line.trim();
  if (/[:：]\s*$/.test(t)) return true;
  if (CHATTER.test(t) && !/\d/.test(t)) return true;
  if (CHATTER.test(t) && /^[a-z\s,.!'-]+$/i.test(t)) return true;
...
export function extractQty(line: string): { qty: number; rest: string } {
  const lead = [
  ...
  for (const re of lead) {
    ...
    if (m) return { qty: parseInt(m[1], 10), rest: line.replace(re, "").trim() };
  }
  const trail: RegExp[] = [
  ...
  for (const re of trail) {
    const m = line.match(re);
    if (m && m.index !== undefined) return { qty: parseInt(m[1], 10), rest: line.slice(0, m.index).trim() };
  }
  const w = line.match(/^\s*(one|two|three|four|five|six|seven|eight|nine|ten|twelve)\s+(?=[a-z])/i);
  if (w) return { qty: WORD_NUM[w[1].toLowerCase()], rest: line.replace(w[0], "").trim() };
...
export function detectSide(text: string): string | null {
  ...
  if (L && !R) return "L/H";
  if (R && !L) return "R/H";
  if (C && !L && !R) return "CENTRE";
  return null;
}

function sideConflict(want: string | null, have: string): number {
  if (!want || !have || have === "-" || have === "BOTH") return 0;
  if (want === have) return 10;
...
export function matchLine(rawLine: string, catalogue: Part[]): Omit<Row, "key" | "chosen" | "status"> {
  ...
  if (side) trace.push({ step: "side lock", detail: `${side} — enforced as a hard gate` });
  if (fired.length) trace.push({ step: "vernacular", detail: fired.join("  ·  ") });
  ...
  if (pnHits.length) trace.push({ step: "part numbers", detail: pnHits.join(", ") });

  const reqTri = trigrams(expanded);

  const scored: Candidate[] = catalogue.map((part) => {
    let score = 0;
    let why = "";

    for (const pn of pnHits) {
      const inv = canonPN(part.inv_pn), cat = canonPN(part.cat_pn), sku = canonPN(part.sku);
      if (pn === inv || pn === cat) { score = Math.max(score, 100); why = `exact PN ${part.inv_pn}`; }
      else if (pn === sku) { score = Math.max(score, 99); why = `SKU ${part.sku}`; }
      else if (basePN(pn) && basePN(pn) === basePN(inv)) { score = Math.max(score, 93); why = `base PN ${basePN(inv)} (suffix differs)`; }
      else if (pn.length >= 6 && (inv.includes(pn) || cat.includes(pn))) { score = Math.max(score, 82); why = `partial PN in ${part.inv_pn}`; }
      else if (inv.length >= 6 && pn.includes(inv)) { score = Math.max(score, 80); why = `PN contains ${part.inv_pn}`; }
    }

    const nameTri = trigrams(`${part.name} ${part.section}`);
    const textScore = Math.round(dice(reqTri, nameTri) * 100);
    if (textScore > score) { score = textScore; why = `text match on "${part.name}"`; }
    else if (textScore > 30 && score < 100) score = Math.min(100, score + Math.round(textScore * 0.06));

    const secNorm = normalise(part.section);
    if (secNorm && expanded.includes(secNorm)) { score += 12; why = why || `section "${part.section}"`; }

    score += sideConflict(side, part.side);
    if (memory.isRejected(rest, part.id)) score -= 60;
...
export function identify(text: string, catalogue: Part[]): Row[] {
  return segment(text).map((line, i) => {
    const r = matchLine(line, catalogue);
    const status: Band = !r.candidates.length
      ? "unmatched"
