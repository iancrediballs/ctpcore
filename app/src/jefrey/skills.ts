...
import {
  Part, Row, canonPN, samePart, normalise, expand, trigrams, relevance,
  extractQty, isChatter,
} from "./brain";
...
export function resolved(rows: Row[]): Array<{ row: Row; part: Part }> {
  return rows
...
export type QuoteLine = {
  qty: number; part: Part; unit: number; ext: number; cost: number;
  margin: number; avail: number; back: number; lead: string; negative: boolean;
};
export type Quote = {
  lines: QuoteLine[]; sub: number; cost: number; vat: number; total: number;
  margin: number; unresolved: number; backordered: number;
  negativeLines: QuoteLine[]; assumptions: string[];
};

export function buildQuote(rows: Row[]): Quote {
  const lines: QuoteLine[] = resolved(rows).map(({ row, part }) => {
...
export type Need = { kind: "unknown" | "shortfall"; qty: number; phrase: string; part: Part | null };
export type RFQ = {
  groups: Record<string, Need[]>; need: Need[];
  unknown: number; shortfall: number; estCost: number; text: string;
};

export function buildRFQ(rows: Row[]): RFQ {
  const need: Need[] = [];
  rows.forEach((r) => {
    const cand = r.candidates[r.chosen];
    if (!cand) { need.push({ kind: "unknown", qty: r.qty, phrase: r.phrase, part: null }); return; }
    const short = r.qty - Math.max(0, cand.part.qty);
    if (short > 0) need.push({ kind: "shortfall", qty: short, phrase: r.phrase, part: cand.part });
  });
...
/* ========================================================================== */
/* 3. GOODS-IN RECONCILIATION                                                 */
/* ========================================================================== */

export type GoodsSev = "ok" | "short" | "over" | "suffix" | "unexpected" | "unknown";
export type GoodsFinding = { sev: GoodsSev; part: Part | null; want: number; got: number; note: string };

function parsePackingList(text: string) {
  return text
    .split(/\r?\n/)
    .map((l) => l.trim())
    ...
    .map((line) => {
      ...
      if (tail && pn && canonPN(tail[1]) !== pn) qty = parseInt(tail[1], 10);
      return { raw: line, pn, qty };
    })
    .filter((x) => !!x.pn) as Array<{ raw: string; pn: string; qty: number }>;
}

export function reconcile(
  packingText: string,
  expectedRaw: Array<{ part: Part; qty: number }>,
  catalogue: Part[]
): { findings: GoodsFinding[]; tally: Partial<Record<GoodsSev, number>>; parsed: number } {
  ...
  const byId = new Map<number, { part: Part; qty: number }>();
  expectedRaw.forEach((e) => {
    const prev = byId.get(e.part.id);
    if (prev) prev.qty += e.qty; else byId.set(e.part.id, { part: e.part, qty: e.qty });
  });

  const findings: GoodsFinding[] = [];
  const seen = new Set<typeof got[number]>();
  ...
  byId.forEach((e) => {
    ...
    if (!hit) findings.push({ sev: "short", part: e.part, want: e.qty, got: 0, note: "line missing from the shipment entirely" });
    else if (hit.qty < e.qty) findings.push({ sev: "short", part: e.part, want: e.qty, got: hit.qty, note: `${e.qty - hit.qty} short` });
    else if (hit.qty > e.qty) findings.push({ sev: "over", part: e.part, want: e.qty, got: hit.qty, note: `${hit.qty - e.qty} over — check before booking it in` });
    else findings.push({ sev: "ok", part: e.part, want: e.qty, got: hit.qty, note: "matches" });

    if (hit && !exact) findings.push({
  ...
  const tally: Partial<Record<GoodsSev, number>> = {};
  findings.forEach((f) => { tally[f.sev] = (tally[f.sev] || 0) + 1; });
  return { findings, tally, parsed: got.length };
}

/* ========================================================================== */
/* 4. PICK LIST & BIN ROUTING                                                 */
/* ========================================================================== */

export type LocKey = { aisle: string; bay: number; level: string; txt: string };
...
export function locKey(loc: string): LocKey {
  const m = (loc || "").match(/^([A-Z]+)(\d+)[-\s]?([a-z])?/i);
  if (!m) return { aisle: "ZZ", bay: 999, level: "z", txt: loc || "—" };
  return { aisle: m[1].toUpperCase(), bay: parseInt(m[2], 10), level: (m[3] || "a").toLowerCase(), txt: loc };
}

export type PickItem = { qty: number; part: Part; loc: LocKey; pick: number; short: number };

export function buildPickList(rows: Row[]) {
  const items: PickItem[] = resolved(rows).map(({ row, part }) => ({
  ...
  items.sort((a, b) =>
    a.loc.aisle.localeCompare(b.loc.aisle) || a.loc.bay - b.loc.bay || a.loc.level.localeCompare(b.loc.level));
...
export type Finding = { sev: Sev; code: string; part: Part | null; detail: string; cost: number };

/** Ranked by what costs money, not by what is easiest to detect. */
export function auditCatalogue(catalogue: Part[]) {
  const f: Finding[] = [];
  const push = (sev: Sev, code: string, part: Part | null, detail: string, cost = 0) =>
    f.push({ sev, code, part, detail, cost });

  const byPN = new Map<string, Part[]>();
  catalogue.forEach((p) => {
    if (!p.inv_pn) return;
  ...
  byPN.forEach((list, pn) => {
    if (list.length > 1)
      push("critical", "DUPLICATE_PN", list[0],
        `${list.length} rows share inventory PN ${pn} (${list.map((x) => x.sku).join(", ")}) — stock counts will disagree`);
  });
  ...
  catalogue.forEach((p) => {
    ...
    if (p.list > 0 && p.cost >= p.list)
      push("critical", "NEGATIVE_MARGIN", p,
        `list ${money(p.list)} is at or below landed cost ${money(p.cost)} — you lose ${money(p.cost - p.list)} a unit`,
        (p.cost - p.list) * Math.max(p.qty, 0));

    if (!p.list) push("high", "NO_PRICE", p, "no list price — cannot be quoted");
    if (!p.loc) push("high", "NO_LOCATION", p, "no warehouse bin — cannot be picked");
    if (p.qty === 0) push("medium", "OUT_OF_STOCK", p, "on the catalogue, none on the shelf");
    else if (p.qty <= LOW_STOCK) push("low", "LOW_STOCK", p, `only ${p.qty} left`);
    if (!p.photo) push("medium", "NO_PHOTO", p, "no photo — sells worse without an image");
...
/* ========================================================================== */
/* 6. ASK — deterministic query engine                                        */
/* ========================================================================== */

export type Answer = { answer: string; sql: string; table: Part[] | null; rule: string };
...
const singularise = (s: string) =>
  s.split(" ").map((w) => (w.length > 4 && /[^s]s$/.test(w) ? w.slice(0, -1) : w)).join(" ");

const search = (q: string, c: Part[], min: number, fields: (p: Part) => string): Part[] => {
...
type Rule = { id: string; test: (q: string) => boolean; run: (q: string, c: Part[]) => Omit<Answer, "rule"> };
...
export function ask(question: string, catalogue: Part[]): Answer {
  const q = singularise(normalise(question));
  const rule = RULES.find((r) => r.test(q)) as Rule;
...
export function draftReply(rows: Row[], quote: Quote | null): string {
  const inStock: Array<{ row: Row; part: Part }> = [];
  const partial: Array<{ row: Row; part: Part; avail: number }> = [];

  resolved(rows).forEach(({ row, part }) => {
    const avail = Math.min(row.qty, Math.max(0, part.qty));
    if (avail === row.qty) inStock.push({ row, part });
    else partial.push({ row, part, avail });
  });
  const sourcing = rows.filter((r) => !r.candidates[r.chosen]);

  const L: string[] = ["Hi,", "", "Thanks for the list — here's where we stand:", ""];
  if (inStock.length) {
  ...
  if (partial.length) {
    ...
    partial.forEach(({ row, part, avail }) =>
      L.push(`  ${row.qty} × ${part.name} (${part.inv_pn}) — ${avail} now, ${row.qty - avail} to follow — ${money(part.list)} ea`));
    L.push("");
  }
  if (sourcing.length) {
...
/* ========================================================================== */
/* 8. SUPERSESSION / CROSS-REFERENCE ADVISOR                                  */
/* ========================================================================== */

export type XRef = { kind: "pair" | "ref" | "suffix" | "hand"; part: Part | null; note: string };
...
export function crossRefs(part: Part, catalogue: Part[]): XRef[] {
  const out: XRef[] = [];
  (part.refs || []).forEach((r) => {
    const t = catalogue.find((p) => samePart(p.inv_pn, r) && p.id !== part.id);
    if (t) out.push({ kind: "pair", part: t, note: `paired with ${t.side !== "-" ? t.side + " " : ""}${t.name}` });
    else out.push({ kind: "ref", part: null, note: `references ${r}, which isn't in the catalogue` });
  });
  ...
  if (part.side === "L/H" || part.side === "R/H") {
    ...
    const strip = (s: string) => normalise(s).replace(/l\/h|r\/h/g, "").replace(/\s+/g, " ").trim();
    const sib = catalogue.find((p) => p.side === want && p.section === part.section && strip(p.name) === strip(part.name));
    if (sib) out.push({ kind: "hand", part: sib, note: `opposite hand: ${sib.inv_pn} (${sib.qty} on hand)` });
  }
  return out;
}
